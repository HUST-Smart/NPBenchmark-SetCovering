...
public:
    ...
    struct Cli {
        ...
        static String InitSolutionPathOption() { return "-is"; }
    ...
    struct Environment {
        ...
        static String DefaultInitSolutionDir() { return "InitSolution/"; }
        ...
        static String DefaultResPath() { return "result.csv"; }

        Environment(const String &instancePath, const String &solutionPath, const String &initsolutionPath,//³õÊ¼½âÂ·¾¶
            ...
            const String &resultFilePath = DefaultResPath())
            : instPath(instancePath), slnPath(solutionPath), initPath(initsolutionPath), randSeed(randomSeed),
            msTimeout(static_cast<Duration>(timeoutInSecond * Timer::MillisecondsPerSecond)), maxIter(maxIteration),
            jobNum(jobNumber), rid(runId), cfgPath(cfgFilePath), logPath(logFilePath), resultPath(resultFilePath), localTime(Timer::getTightLocalTime()) {
        }
...
public:
    #pragma endregion Constant
            // typedef struct solutionUnit SolutionUnit;
    struct solutionUnit {
        char isESolution;
        int data;
        struct solutionUnit * nextLayerHead;
        struct solutionUnit * nextBrother;
    };
    solutionUnit * TabuTenure;

    #pragma region Constructor
public:
    Solver(const Problem::Input &inputData, const Problem::Output &outputData, const Environment &environment, const Configuration &config)
        : input(inputData), initoutput(outputData), env(environment), cfg(config), rand(environment.randSeed),
...
protected:
    void init();
    bool optimize(Solution & sln, ID workerId);
    set<ID> delSets;
...
public:
    ...
    struct {
        int objColumnNum;
        std::vector<std::vector<int>> ColumnGather;
        std::vector<std::vector<int>> RowGather;
       // Arr2D<Length> ColumnGather;
        //Arr2D<Length> RowGather;
    }Data;
